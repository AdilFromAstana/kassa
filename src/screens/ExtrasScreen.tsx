...
const TOOLS = [
  { label: 'Настройки терминала', note: 'параметры кассы' },
      ...
      <div className="flex-1 overflow-auto p-6">
        {kind === 'tools' && (
          <div className="mb-5 max-w-3xl w-full grid grid-cols-2 gap-3">
            <button onClick={() => navigate('/settings')} className="bg-white/5 hover:bg-white/10 rounded-lg p-4 text-left">
              <div className="text-pos-accent text-sm uppercase mb-1">Настройки заведения ›</div>
              <div className="text-xs text-white/50">Тип ({establishment.mode === 'restaurant' ? 'Ресторан' : 'Фастфуд'}), режим, функции, число ФР. Управляет видимостью кнопок по всему фронту.</div>
            </button>
        ...
        <div className="grid grid-cols-2 gap-3 max-w-3xl">
          {items.map((it) => {
            ...
            return (
            <button key={it.label}
              onClick={() => (pr ? printToast(pr) : alert(`${it.label} — раздел появится при подключении модуля/бэкенда`))}
