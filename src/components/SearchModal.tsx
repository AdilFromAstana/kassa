...
import { usePos, isStopped } from '../store/pos'
  ...
  return (
    <div className="absolute inset-0 bg-pos-bg/95 z-40 flex flex-col">
        ...
        <div className="max-w-2xl mx-auto">
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Enter' && results.length === 1 && !isStopped(stopList,results[0].id)) onPick(results[0])
            }}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {results.map((d) => {
              const stopped = isStopped(stopList,d.id)
              return (
      ...
      <div className="h-14 bg-[#1a1a1a] text-white flex items-center justify-end px-6 gap-6">
        ...
        <button onClick={() => { if (results.length === 1 && !isStopped(stopList,results[0].id)) onPick(results[0]) }}
